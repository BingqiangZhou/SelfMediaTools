...
**封面策略**: 抖音创作者平台在上传视频后会提供封面编辑功能，可以选择视频中的任意一帧作为封面。
...
2. **查找封面编辑入口**:
   - 使用 `take_snapshot` 获取页面快照
   - 查找包含"封面"、"编辑封面"、"选择封面"等文字的按钮或链接
   - 可能的元素文本: "编辑封面"、"选择封面"、"封面设置"、"更换封面"

3. **进入封面编辑模式**:
   ```bash
   mcp__chrome-devtools__click(uid="封面编辑按钮uid")
   ```
   - 点击后会打开封面选择器，显示视频预览和时间轴

4. **选择视频第一帧作为封面**:
   - **方法1 - 直接选择**: 如果时间轴可以选择，将滑块移动到最左边（00:00位置）
     ```bash
     mcp__chrome-devtools__click(uid="时间轴起始位置uid")
     ```
   - **方法2 - 使用预设选项**: 某些版本会提供"视频首帧"作为预设选项
     ```bash
     mcp__chrome-devtools__click(uid="首帧选项uid")
     ```
   - **方法3 - 精确输入**: 如果有时间输入框，输入 "00:00" 或 "0"
     ```bash
     mcp__chrome-devtools__fill(uid="时间输入框uid", value="00:00")
     ```

5. **确认封面选择**:
   - 查找"确定"、"保存"、"确认"等按钮
   ```bash
   mcp__chrome-devtools__click(uid="确认按钮uid")
   ```

6. **验证封面已选择**:
   - 使用 `take_snapshot` 检查封面预览是否显示视频第一帧
   - 确认封面缩略图显示正确

**注意事项**:
- 如果抖音平台默认已使用视频首帧作为封面，且显示正常，可跳过编辑步骤
- 封面选择功能可能因抖音平台版本更新而变化，根据实际页面元素调整操作
- 如果找不到封面编辑入口，可能需要先完成视频上传后再进行封面设置
...
2. **填写文案内容**（原始文案 + 话题标签）:
   - 查找文案输入区域（包含"文案"、"作品描述"等提示）
   - **重要**: 填入完整原始文案内容（不包含标题），并添加话题标签：
   ```bash
   mcp__chrome-devtools__fill(uid="文案输入框uid", value="原始文案内容 #话题1 #话题2")
   ```
